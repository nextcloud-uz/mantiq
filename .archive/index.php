...
<body class="bg-gray-100 min-h-screen font-sans text-gray-800">
                ...
                <div id="progressContainer" class="hidden w-full pt-4">
                    <div class="flex justify-between text-sm font-semibold text-blue-600 mb-2">
                        <div class="flex items-center gap-2">
                            ...
                            <span id="progressText">Navbatga qo'yilmoqda...</span>
                        </div>
    ...
    <script>
        ...
        form.addEventListener('submit', async (e) => {
            ...
            updateProgress(5, 'Fayl GPU serveriga yuborilmoqda...');
            ...
            try {
                ...
                // 2-QADAM: HOLATNI TEKSHIRISH (POLLING)
                updateProgress(20, 'Task qabul qilindi. Jarayon kutilmoqda...');
        ...
        function checkTaskStatus(taskId, requestedFormat) {
            checkInterval = setInterval(async () => {
                try {
                    ...
                    // Odatda done, success, finished bo'lishi mumkin
                    const status = data.task_status ? data.task_status.toLowerCase() : 'pending';

                    // API qancha doc yuborilganini aytsa real foiz chiqaramiz
                    if (data.task_meta && data.task_meta.num_docs > 0) {
                        const percent = Math.floor((data.task_meta.num_processed / data.task_meta.num_docs) * 100);
                        updateProgress(Math.max(30, percent), `Tahlil qilinmoqda... Sahifalar: ${data.task_meta.num_processed}/${data.task_meta.num_docs}`);
                    } else {
                        // Agar ma'lumot qaytmasa o'zimiz sekin ko'taramiz 90% gacha
                        let currentW = parseInt(progressBar.style.width) || 30;
                        if (currentW < 90) updateProgress(currentW + 5, 'AI modeli PDFni tahlil qilmoqda...');
                    }

                    // TUGADI -> Natijani olish
                    if (status === 'success' || status === 'completed' || status === 'finished' || status === 'done') {
                        clearInterval(checkInterval);
                        updateProgress(95, 'Tahlil tugadi! Natija tortib olinmoqda...');
                        fetchFinalResult(taskId, requestedFormat);
                    }
                    // XATOLIK
                    else if (status === 'failed' || status === 'error') {
                        clearInterval(checkInterval);
                        throw new Error("Docling jarayonida serverda xatolik yuz berdi!");
                    }
            ...
            }, 2000); // 2 soniya kuttiramiz
        }

        // 3-QADAM: NATIJANI OLIB KELISH
        async function fetchFinalResult(taskId, format) {
            try {
                ...
                updateProgress(100, 'Muvaffaqiyatli yakunlandi!');
